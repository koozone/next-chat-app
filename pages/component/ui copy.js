...
const getStyle = (props) => {
	const {tag, css, deco, icon, selected} = props;
	...
	const baseElement = (() => {
		switch (`[${tag}]`) {
			...
			case '[button]':
				return (
					<div
						className={`group px-2 py-1 ring-1 ring-black/30 hover:ring-2 text-sm leading-6 font-medium rounded-md space-x-1 ${selected ? '' : ''}`}
					/>
				);
	...
	// label: <div className="cursor-text space-x-1" />,
	...
	return `${baseElement.props.className} | ${element.props.className}`;
};
...
const getContent = (props) => {
	...
	return (
		<>
			{icon || iconL ? <i className={`bx ${icon || iconL} align-middle`} /> : ''}
			{name ? <span className="align-middle">{name}</span> : ''}
			{iconR ? <i className={`bx ${iconR} align-middle`} /> : ''}
		</>
	);
};
