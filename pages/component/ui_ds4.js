...
const Type = (props) => {
	...
	return props.tag == 'input' ? (
		...
		<input type={type} className={`${style} | ${className}`} id={id} name={name} onChange={onChange} checked={checked} disabled={disabled} />
	);
};
const Type2 = forwardRef((props, ref) => {
	...
	return disabled ? (
		...
		<input type={type} className={`${style} | ${className}`} id={id} name={name} ref={ref} onChange={onChange} value={value} placeholder={placeholder} />
	);
});
...
export const Basket = (props) => {
	...
	const {children, deco, style, className, icon, iconL, iconR, name, text, id, type, onChange, checked = false, disabled} = newProps;
